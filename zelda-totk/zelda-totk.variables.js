...
class Struct {
    constructor(structId, variablesInfo, structArray) {
...
class Variable {
    constructor(hashText, type, enumValues) {
    ...
    static _onChangeInputNumberFix(evt) {
    ...
    static _save(offset, type, value, bitIndex) {
        if (type === 'BoolArray' && typeof bitIndex === 'number') {
    ...
    static _buildHtmlInput(variable, inputType, defaultValue, autosave, enumValues, arrayIndex) {
        var input;
    ...
    static toHexString(u32) {
        var str = u32.toString(16);
        while (str.length < 8)
            str = '0' + str;
        return '0x' + str;
    }
    static enumToInt(param, defaultValue) {
    ...
    updateHtmlInputValue(arrayIndex) {
        if (!this._htmlInputs)
            return false;
