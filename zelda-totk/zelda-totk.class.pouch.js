...
class Pouch {
    ...
    static getCategoryStruct(catId) {
        if (catId === 'weapons') {
    ...
    static _onChangeInputTextFix(evt) {
    ...
    static updateItemIcon(item) {
        item._htmlIcon.src = Pouch.getItemIcon(item);
    }
    static updateItemRow(item) {
        if (!item._htmlRow) {
    ...
    static createItemInput(item, propertyName, type, options) {
